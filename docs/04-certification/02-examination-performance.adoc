...
Die Prüfer führen ein 30-minütiges Interview mit jedem einzelnen Mitglied der Themenarbeitsgruppe durch, um zu überprüfen, ob es die Inhalte verstanden und sich entsprechend in die Themenarbeitsgruppe eingebracht hat. Um die Zertifizierung zu erhalten, muss ein Mitglied die Beurteilung "Herausragend" oder "Verstanden" erhalten. Dabei gelten diese Kriterien:

include::../05-criteria/08-criteria-interview.adoc[{include_configuration}]
...
The examiners interview each member of the working group to make sure they understand the topic and worked sufficiently on the results. A member must get the rating "Outstanding" or "Understood" to get the certification. The following criteria are used to determine the rating.
